...
public class GuessOrc {
	...
	private void play() {
		Integer guess = 0;
		do {
			guess = getNextInput(movesMade() == 0);
			Integer hint = calculateHint(guess);
			printMap(guess, hint);
		} while (movesMade() < MAX_ATTEMPTS && (guess != mineShaftId));

		end(guess == mineShaftId);
	}
	...
	public int getNextInput(boolean isInitialGuess) {
		Integer guessInput = -1;

		do {
			System.out.print(isInitialGuess ? "Please enter your first guess: " : "Next guess? ");
			String guessStr = input.nextLine();
			guessInput = getColumnAsInt(guessStr.charAt(0));
		} while (guessInput == -1);
	...
	public int calculateHint( int guessedMineShaftId ){
		...
		return possibleHints.get(generator.nextInt(possibleHints.size()));
	}
